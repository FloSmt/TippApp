...
describe('TipSeasonService', () => {
  ...
  it('should return a TipSeason-Object with correct content', () => {
    ...
    const matchDays: GroupResponse[] = [
      { groupId: 1, groupName: 'Matchday 1', groupOrderId: 1 },
      { groupId: 2, groupName: 'Matchday 2', groupOrderId: 2 },
    ];
    const matches = [
      { matchId: 101, group: { groupId: 1 } },
      { matchId: 102, group: { groupId: 1 } },
      { matchId: 201, group: { groupId: 2 } },
    ] as unknown as MatchResponse[];

    const tipSeason = service.createTipSeason(
      season,
      matchDays,
      matches,
      mockEntityManager
