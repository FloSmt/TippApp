...
export class AuthService {
  ...
  async login(loginDto: LoginDto): Promise<{ accessToken: string; refreshToken: string }> {
    ...
    if (!user) {
      throw this.errorManager.createError(ErrorCodes.Auth.USER_NOT_FOUND, HttpStatus.NOT_FOUND);
    }

    const isPasswordMatch = await this.hashService.comparePasswords(loginDto.password, user.password);
    if (!isPasswordMatch) {
      throw this.errorManager.createError(ErrorCodes.Auth.INVALID_CREDENTIALS, HttpStatus.UNAUTHORIZED);
    }
  ...
  async register(registerDto: RegisterDto): Promise<{ refreshToken: string; accessToken: string }> {
    ...
    if (userAlreadyExists) {
      throw this.errorManager.createError(ErrorCodes.Auth.EMAIL_ALREADY_EXISTS, HttpStatus.CONFLICT);
    }
    const passwordHash = await this.hashService.hashPassword(registerDto.password);
